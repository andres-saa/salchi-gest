<template>
    <div class="p-3" style="min-width: 100%; margin: auto;">
        ...
        <h2 class="my-6"><b>Ticket Promedio por canal</b></h2>
        ...
        <Chart :type="type_graph.reporte_basico.value" :data="store.TicketDataGraphics.reporte_basico" class="h-[10rem] p-2 h-50"
            style="width: 100%; box-shadow: 0 0 10px rgba(0, 0, 0, .2); border-radius: .5rem;" />


        
            <h2 class="my-6"><b>Ticket Promedio Total periodo actual vs periodo anterior</b></h2>




            <Chart type="line":data="store.TicketDataGraphics.total" class="h-[10rem] p-2 h-50"
            style="width: 100%; box-shadow: 0 0 10px rgba(0, 0, 0, .2); border-radius: .5rem;" />



            <h2 class="my-6"><b>Ticket Promedio Call center periodo actual vs periodo anterior</b></h2>
        ...
        <Chart type="line" :data="store.TicketDataGraphics.callcenter" class="h-[10rem] p-2 h-50"
            style="width: 100%; box-shadow: 0 0 10px rgba(0, 0, 0, .2); border-radius: .5rem;" />
        ...
        <h2 class="my-6"><b>Ticket Promedio Salchibot periodo actual vs periodo anterior</b></h2>

        <Chart type="line" :data="store.TicketDataGraphics.chatbot" class="h-[10rem] p-2 h-50"
            style="width: 100%; box-shadow: 0 0 10px rgba(0, 0, 0, .2); border-radius: .5rem;" />
        ...
        <h2 class="my-6"><b>Ticket Promedio Pagina web periodo actual vs periodo anterior</b></h2>

        <Chart type="line" :data="store.TicketDataGraphics.web" class="h-[10rem] p-2 h-50"
            style="width: 100%; box-shadow: 0 0 10px rgba(0, 0, 0, .2); border-radius: .5rem;" />
    </div>
</template>
...
// const store.TicketDataGraphics = ref([])

watch(() => store.dateRange, async () => {
    const site_ids = store.selectedSites.map(s => s.site_id)
    store.TicketDataGraphics = await fetchService.post(`${URI}/get_daily_average_ticket_report/`, {
