...
<Dialog class="mx-2" v-model:visible="showAddNotesDialog" modal header="Notas de alistamientos"
        :style="{ width: '25rem' }">
    ...
    <Dialog class="mx-2" v-model:visible="showActionDialog" modal header="Interactuar con la orden"
        ...
        <div class="m-0 p-0"
            ...
            <Button @click="openPrepare(dataToInract.purchase_order_id)" label="Autorizar esta orden" style=""
    ...
    <Dialog class="px-2" v-model:visible="showPrepare" modal header="Autorizar orden"
        ...
        <div class="m-0 p-0"
            ...
            <p class="m-0 p-0"><b>Alistada por:</b> {{ dataToInract.responsible_lap   }} </p>
            <p class="m-0 p-0"><b>Sede:</b> {{ dataToInract.site_name }} </p>
            <p class="mt-0 p-0"><b>Fecha de Alistamiento:</b> {{ dataToInract.status_timestamp.split('T')[0] }} </p>
            ...
            <DataTable showGridlines style="max-width: 1024px;" stripedRows v-model:filters="filters"
                ...
                <Column class="py-0 " field="quantity" header="cant. Solicitada por la sede">
                    <template #body="data">
                <p class="text-right">
                    {{ data.data.quantity }}
                </p>
                ...
                <Column class="py-0 px-0" field="quantity" header="cant. despachada por quien alista">


                    <template #body="data">
                        <input readonly  @keydown="handleKeydown" class=" text-right"
                            :max="data.data.quantity" :min="0" v-model="data.data.quantity_adjusted"
                            style="background-color: transparent; height: 2rem; width:100%; border: none;"
                            :style="data.data.quantity === data.data.ajustment && data.data.quantity_adjusted < data.data.quantity ? 'background-color: aquamarine;' : data.data.ajustment === 0 ? 'background-color:#ffc9d1' :  
                                data.data.quantity_adjusted < data.data.quantity ? 'background-color:#FFAF33;border:2px solid #FF7A33' : ''"
                            type="number" />
                    </template>
                ...
                <Column class="py-0" field="quantity" header="interactuar">
                    ...
                    <template #body="data">
                        <Button v-if="data.data.quantity_adjusted < data.data.quantity" @click="data.data.ajustment = data.data.quantity" style="min-width: max-content;" text
                            class="px-0 py-1" label="Aprobar"></Button>
                    </template>
        ...
        <template #footer>
            <div class="pt-3" style="display: flex;justify-content: end;align-items: end;">
                <Button outlined @click="complete_all" severity="help" label="Aprobar todo"></Button>
    ...
    <div class="mt-3" style="min-height:20vh; display: flex; justify-content:center;align-items:center">
        ...
        <DataTable showGridlines style="max-width: 1024px;" v-model:filters="filters" class="col-12 m-auto"
            ...
            <template #header>
                <div class="grid" style="align-items:center">
                    <div class="col-12 md:col-6 p-3">
                        <span class="text-xl" style="width: 100%;"> Ordenes pendientes por Autorizar</span>
                    </div>
                    <div class="col-12 md:col-6 p-3">
            ...
            <Column class="py-1" field="responsible_lap" header="Quien la alisto"></Column>
...
const complete_all = () => {
    itemsToPrepare.value.forEach(element => {
        element.ajustment = element.quantity
    });
}
...
const sendData = async (itemsData) => {
    ...
    if (itemsData.some(item => !item.ajustment && item.ajustment !== 0 && item.quantity_adjusted < item.quantity)) {
        alert("Debe aceptar los ajustes antes de Autorizar")
        return
    }
    ...
    loadingStore.setLoading(true,'Autorizando orden')
    const dataToSend = {
    ...
    loadingStore.setLoading(false,'Autorizando orden')
