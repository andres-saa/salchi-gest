<template>
    ...
    <Dialog class="mx-2" v-model:visible="showActionDialog" modal header="Interactuar con la orden"
        ...
        <div class="m-0 p-0"
            ...
            <Button @click="openPrepare(dataToInract.purchase_order_id)" label="Alistar esta orden" style=""
    ...
    <Dialog class="px-2" v-model:visible="showPrepare" modal header="Alistar orden"
        ...
        <div class="m-0 p-0"
            ...
            <p class="m-0 p-0"><b>Solicitante:</b> {{ dataToInract.employer_name }} </p>
            <p class="m-0 p-0"><b>Sede:</b> {{ dataToInract.site_name }} </p>
            <p class="mt-0 p-0"><b>Fecha de Expedicion:</b> {{ dataToInract.expedition_date.split('T')[0] }} </p>
            ...
            <DataTable showGridlines style="max-width: 1024px;" stripedRows v-model:filters="filters"
                ...
                <Column class="py-0 text-right" field="quantity" header="cant. Solicitada"></Column>

                <Column class="py-0 px-0" field="quantity" header="cant. a enviar">
                    ...
                    <template #body="data">
                        <input  @keydown="handleKeydown" class=" text-right"
                            ...
                            :style="data.data.quantity === data.data.ajustment ? 'background-color: aquamarine;' : data.data.ajustment === 0 ? 'background-color:#ffc9d1' :  
                                data.data.ajustment > data.data.quantity ? 'background-color:red' : ''"
                            type="number" />
                    </template>
                ...
                <Column class="py-0" field="quantity" header="interactuar">
                    ...
                    <template #body="data">
                        <Button @click="data.data.ajustment = data.data.quantity" style="min-width: max-content;" text
                            class="px-0 py-1" label="Enviar completo"></Button>
                    </template>
        ...
        <template #footer>
            <div class="pt-3" style="display: flex;justify-content: end;align-items: end;">
                <Button outlined @click="complete_all" severity="help" label="Igualar todo"></Button>
...
import { da, es, tr } from 'date-fns/locale';
...
const sendData = async (itemsData) => {
    ...
    loadingStore.setLoading(true,'Alistando orden')
    const dataToSend = {
    ...
    loadingStore.setLoading(false,'Alistando orden')
