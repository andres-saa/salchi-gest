...
<div class="mt-3 mx-auto" style="min-height:20vh;max-width:1024px; display: flex; justify-content:center;align-items:center">
    ...
    <DataTable stripedRows style="max-width: 1024px;"  v-model:filters="filters" class="col-12 m-auto" :value="invetnoryMonthlyReports" tableStyle="min-width: 50rem;">
        <template #header>
        ...
        <Column class="py-1" field="monthly_inventory_id" header="ID"></Column>
        ...
        <Column style="max-width: 5rem" class="py-1 px-0 m-0" field="date" header="Action">
            <template #body="data">
                ...
                <div style="display: flex;gap:0.5rem;">
                    <router-link :to="`/monthly-inventory/monthly-inventory-view/${data.data.monthly_inventory_id}`">
                        ...
                        <Button @click="prepareDownload(data.data.monthly_inventory_id,data.data.site_name,data.data.date)" style="height: 2rem;color:white; background:var(--primary-color);border:none" class="p-1"
                        icon="pi pi-download" />
...
import { monthlyInventoryReportsService } from '../../../../service/inventory/monthlyInventoryService.js'
...
const invetnoryMonthlyReports = ref()
const filters = ref();
...
onMounted(async () => {
    ...
    invetnoryMonthlyReports.value = await monthlyInventoryReportsService.getAllMyMonthlyInventoryReports(user_id)
})
...
const getFiltered = async() => {
    ...
    invetnoryMonthlyReports.value = await monthlyInventoryReportsService.getAllMyMonthlyInventoryReportsFiltered(user_id,new_startDate,new_endDate)
}
...
const prepareDownload = async (monthly_inventory_id,site_name,date) => {
    entries.value = await monthlyInventoryReportsService.getMonthlyInventoryEntriesByMonthlyInventoryID(monthly_inventory_id)
...
const downloadAll2 = async () => {
    ...
    const reportes = invetnoryMonthlyReports.value;
    const entries = [];

    for (const reporte of reportes) {
        const reports = await monthlyInventoryReportsService.getMonthlyInventoryEntriesByMonthlyInventoryID(reporte.monthly_inventory_id);
