...
const prepareItemsToSend = (grupo) => {
    return grupo.map(grupo => grupo).map(item => 
        ...
        return {
            monthly_inventory_item_id:it.item_id,
            quantity:it.quantity,
            monthly_inventory_unit_measure_id:it.unit_measure_id
        }
    })
}
...
const preareInventory = () => {
    return {
        ...
        monthly_inventory_items:prepareItemsToSend([...groupWithItems.value])
    }
}
...
const openDialog = () => {
    ...
    if(data.monthly_inventory_items.some(item => item.quantity === null)){
...
const sendInventory = async() => {
    ...
    const data = preareInventory()
    data.monthly_inventory_items = prepareItemsToSendCero([...groupWithItems.value])
    await monthlyInventoryReportsService.InsertMonthlyInventory(data)
    router.push('/monthly-inventory/monthly-inventory-my-reports/')
...
onMounted(async () => {
    const grupos = await monthlyInventoryReportsService.getGroupsWithItems()
