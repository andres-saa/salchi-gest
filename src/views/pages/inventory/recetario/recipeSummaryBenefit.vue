<template>
    ...
    <div style="margin-top: 3rem;">
        ...
        <template #header>
            <div class="grid pt-8 px-3" style="align-items:center;justify-content: space-between; display: flex;gap: 1rem;">
                <h3 class="px-0"> <i class="fa fa-chart-column"></i>  <b>Resumen de Beneficios</b></h3>
        ...
        <column frozen header="Foto" class="py-1 px-0" style="max-width: 4rem;" >
            <template #body="data">
                <div style="display: flex; justify-content: center;">
                    <img class="p-0 shadow-2" style="width: 2rem;border-radius: .3rem;background-color: white;" :src="`${URI}/read-product-image/96/${data.data.name}`" alt="">

                </div>
            </template>
        </column>

        <column :frozen="column.fixed" v-for="column in columns" :header="column.header" :style="`min-width:${column.width}rem`"  :field="column.value" class="py-2" >
            <template #body="data" >
                ...
                <h6 v-else-if="column.type != 'percent'" style="text-transform: lowercase;" :style="column.type == 'max-content' ? 'min-width:max-content' : ''" class="my-0 p-0">{{ data.data[column.value] || '-----------'}}</h6>
                <h6 v-if="column.type == 'percent'  " style="text-transform: lowercase;" :style="column.type == 'max-content' ? 'min-width:max-content' : ''" class="my-0 p-0">{{`${data.data[column.value]}%` || '-----------'}}</h6>
...
const columns = [
    ...
    {
        header:'Precio de venta al publico',
        value:'selling_price',
        width:'10',
        type:'money'
    },
    ...
    {
        header:'Precio de venta neto',
        value:'net_selling_price',
        width:'10',
        type:'money'
    },

    {
        header:'Costo total de la receta',
        value:'recipe_total_cost',
        width:'10',
        type:'money'
    },
    ...
    {
        header:'% Costo total de la receta',
        value:'percent_recipe_total_cost',
        width:'10',
        type:'percent'
    },

    {
        header:'Margen de beneficio neto',
        value:'net_benefit_margin',
        width:'10',
        type:'money'
    },

    {
        header:'Margen de beneficio',
        value:'benefit_margin',
        width:'10',
        type:'percent'
    },
...
const update = async() => {
   ingredients.value = await fetchService.get(`${URI}/get-recipe-summary-benefit`,'cargando ingredientes')
