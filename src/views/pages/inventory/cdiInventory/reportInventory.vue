...
const prepareItemsToSend = (grupo) => {
    return grupo.map(grupo => grupo).map(item => 
        ...
        return {
            cdi_inventory_item_id:it.item_id,
            quantity:it.quantity,
            cdi_inventory_unit_measure_id:it.unit_measure_id
        }
    })
}
...
const preareInventory = () => {
    return {
        ...
        cdi_inventory_items:prepareItemsToSend([...groupWithItems.value])
    }
}
...
const openDialog = () => {
    ...
    if(data.cdi_inventory_items.some(item => item.quantity === null)){
...
const sendInventory = async() => {
    ...
    const data = preareInventory()
    data.cdi_inventory_items = prepareItemsToSendCero([...groupWithItems.value])
    await cdiInventoryReportsService.InsertCdiInventory(data)
    router.push('/cdi-inventory/cdi-inventory-my-reports/')
...
onMounted(async () => {
    const grupos = await cdiInventoryReportsService.getGroupsWithItems()
