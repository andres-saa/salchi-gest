...
<div class="mt-3 mx-auto" style="min-height:20vh;max-width:1024px; display: flex; justify-content:center;align-items:center">
    ...
    <DataTable stripedRows style="max-width: 1024px;"  v-model:filters="filters" class="col-12 m-auto" :value="invetnoryCdiReports" tableStyle="min-width: 50rem;" :paginator="true"
            :rows="15"
            ...
            <template #body="data">
                ...
                <div style="display: flex;gap:0.5rem;">
                    <router-link :to="`/cdi-inventory/cdi-inventory-view/${data.data.cdi_inventory_id}`">
                        ...
                        <Button @click="prepareDownload(data.data.cdi_inventory_id,data.data.site_name,data.data.date)" style="height: 2rem;color:white; background:var(--primary-color);border:none" class="p-1"
                        icon="pi pi-download" />
...
onMounted(async () => {
    ...
    invetnoryCdiReports.value = await cdiInventoryReportsService.getAllMyCdiInventoryReports(user_id)
})
...
const getFiltered = async() => {
    ...
    invetnoryCdiReports.value = await cdiInventoryReportsService.getAllMyCdiInventoryReportsFiltered(user_id,new_startDate,new_endDate)
}
