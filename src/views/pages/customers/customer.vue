...
<div style="min-width: 1200px;margin-top: 3rem;">
        ...
        <!-- <nav class="nav_bar shadow-2 p-0 my-0 mx-3" style="position: sticky;top: 6rem;max-width: 1200px; background-color: white;z-index: 99;">
            <ul class="nav_bar--buttons p-0 m-1" style="">
           
                    <li v-for="button in nav_buttons" key="" class="" style="display: flex;align-items: center  ;" >
                     
                       
                       <Tag :style="`background-color:${button.color}`"  style="height: 1.3rem;aspect-ratio: 1 / 1;border-radius: 50%;"></Tag> <Button  @click="() => {active_button_nav = button ; login.currentSection_users = options[0] }" class="nav_bar--buttons-button p-2" :class="button.id == active_button_nav.id? 'nav_bar--buttons-button-selected': ''" :label="button.name"></Button>
       ...
       <div style="display: flex; justify-content: center;width: 100%;align-items: end;gap: 1rem;" class="mt-0 px-1">


        <div class="my-0 px-0"       style="display: flex;transition: all ease .5s; align-items: end; gap: 1rem">
         ...
         <h6><b>Hasta</b> </h6>
         <Calendar style="max-width:7.5rem;"  v-model="temp_end_data"     dateFormat="dd-mm-yy"  
         ></Calendar>
     ...
     <Button @click="getReports(temp_start_date  , temp_end_data)" style="min-width: max-content;" severity="help" label="Buscar" icon="pi pi-search"></Button>
     <Button @click="downloadExcel" style="min-width: max-content;" severity="help" label="Descargar Excel" icon="pi pi-download"></Button>
        ...
        <div class="px-0    mt-2" style="display: flex;width: 100%;align-items: center; justify-content:end;gap: 1rem;">
        ...
        <div class="scroll-container" style="">
            ...
            <DataTable :style="`transform:translateX(${ login.currentSection_users.x}%);transition: .5s all ease;`"  :paginator="true" :rows="15" style="width: 100%;"
           ...
           :value="customers" tableStyle="min-width: 50rem;" :filters="filters">
           <template #header>
               <div class="grid p-3" style="align-items:center;justify-content: space-between; display: flex;gap: 1rem;">
                   <h4 class="px-2"> <i class="fa-regular fa-bars-progress"></i>  <b style="text-transform: uppercase;">USUARIOS <b>{{ route.params.section }}</b> </b> </h4>
           ...
           <column    #body="contract" :header="column.columnName" :style="`min-width:${column.size}`" v-for="column in dataColumns" :field="column.columnValue" class="my-0 py-2">
            ...
            <div class="p" v-if="column.columnValue == 'orders'" style="display: flex;gap: .1rem;flex-direction:column;align-items:center;" >
               
                <div v-for="(i,index) in contract.data[column.columnValue]" :style="`background-color:${pastel_colors[index]}`"   style="padding: .5rem;box-shadow:0 0 1rem #00000020;border-radius: .5rem; margin: .3rem;width: max-content;">
                    <h6 class="m-0"><strong>ID: </strong>{{ i.id_order }}</h6>
                    <h6 class="m-0"><strong>FECHA: </strong>{{ i.fecha_order }}</h6>
                    <h6 class="m-0"><strong>VALOR: </strong>{{ formatoPesosColombianos(i.valor_orden ) }}</h6>
                </div>
               ...
               <h6 style="text-align: start;min-width: max-content;" v-if="column.columnType == 'order_id'" class="m-0 p-0">
                   {{ contract.data[column.columnValue] || '---------'  }}
               </h6>
    
    
               <h6 style="text-align: start;" v-if="column.columnType == 'other'" class="m-0 p-0">
                   {{ contract.data[column.columnValue] || '---------'  }}
               </h6>
...
const  pastel_colors = [
    ...
    // Mapeo de los datos para el Excel
    const data = customers.value.map(item => {
        return {
            "ID de Usuario": item.user_id || '---',
            "Nombre": item.user_name || '---',
            "Teléfonos": item.phone_numbers || '---',
            "Dirección": item.user_address || '---',
            "No. Compras": item.times_purchased !== undefined ? item.times_purchased : '---',
            "Total Gastado": item.total_spent ? formatoPesosColombianos(item.total_spent) : 0,
            "Primera Compra": item.joined_date || '---',
    ...
    // Generar un buffer para el archivo Excel
    ...
    saveAs(blob, 'Reporte_de_Usuarios.xlsx');
};
...
// }
    ...
    //     login.currentSection_users = options[0]
    // })
    ...
    const getReports = async(starD, endD) => {
        ...
        // customers.value = await fetchService.post(`${URI}/customers/${start}/${end}`,{ids:selected_sites.value.map(s => s.site_id)})
