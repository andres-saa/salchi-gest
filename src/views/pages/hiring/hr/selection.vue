<template>
    ...
    <DataTable :paginator="true" :rows="15" style="width: 100%;max-width: 1280px;"
         ...
         <template #header>
             <div class="grid p-3" style="align-items:center;justify-content: space-between; display: flex;gap: 1rem;">
                 <h4 class="px-2 py-5"> <i class="fa-regular fa-bars-progress"></i>  <b>Solicitudes que ya iniciaron la seleccion de personal</b></h4>
         ...
         <column #body="contract" :header="column.columnName" v-for="column in dataColumns" :field="column.columnValue" class="my-0 py-0">
             ...
             <Tag class="p-2" v-if="column.columnValue == 'lap_name'"  style="color: white;" :style="`background-color:${tags.find(tag => tag.name == contract.data.lap_name)?.backgroundColor }`">
                 {{ contract.data[column.columnValue]}}
             </Tag>
         ...
         <template #body="data" >
            ...
            <div style="display: flex;flex-direction: column; gap:.1rem">
                ...
                <router-link to="/hiring-hr-follow/:vacancy_id">
                    <Button style="min-width: max-content" size="small" @click="chargeHistory(data.data.lap_history)" label="seguimiento" severity="help" icon="pi pi-check p-0" class="my-0 py-1"></Button>

                </router-link>
                 <!-- <Button style="min-width: max-content" size="small" @click="chargeHistory(data.data.lap_history)" l   abel="Rechazar solicitud" severity="danger" icon="pi pi-times p-0" class="my-0 py-1"></Button> -->
            </div>
         </template>
 ...
 const tags = [
     ...
     { id: 2, name: "AAprobado por Gerente General", color: "#FFFFFF", backgroundColor: "#2980B9" },    // Azul oscuro
     ...
     { id: 12, name: "Entregado a Recursos Humanos", color: "#FFFFFF", backgroundColor: "#3498DB" }, // Azul brillante
