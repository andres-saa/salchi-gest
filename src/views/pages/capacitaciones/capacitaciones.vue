...
<p class="" style="font-weight: ;"> {{ user.name }}</p>
    ...
    <DataTable ref="dt" :value="capacitaciones" v-model:selection="selectedCapacitaciones" dataKey="id" :paginator="true"
...
const openNew = ref(false)
    ...
    const getCapacitaciones = async () => {
        ...
        try {
            const response = await fetch(`${URI}/trainings/by-creator/${userid}`);
            if (!response.ok) {
