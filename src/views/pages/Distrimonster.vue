<template>
  ...
  <div style="margin-top: 10rem; max-width: 1200px; margin: 10rem auto;" class="px-3">
    ...
    <DataTable @rowReorder="onRowReorder"  :value="data" class="shadow-1">
      <Column v-if="modificando" rowReorder headerStyle="width: 3rem" :reorderableColumn="false" />
...
// Si el evento trae el nuevo arreglo, se puede hacer:
if(event.value){
  data.value = event.value;
}
// Actualiza el índice en cada elemento
...
// Se utiliza slice() para evitar mutar el array original
return data.value.slice().sort((a, b) => a.index - b.index);
});
