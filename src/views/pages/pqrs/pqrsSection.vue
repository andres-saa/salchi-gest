<template>
    ...
    <Dialog class="mx-2 " v-model:visible="showHistoryDialog" modal :header="`Historial de la pqr  ID   ${currentOrderHistory.pqr_request_id}`" :style="{ width: '40rem',  }">
        ...
        <div class="px-2    mt-5" style="display: flex;width: 100%;align-items: end; justify-content:center;gap: 1rem;">
    
    
            <div class="my-0 px-3" :style="login.currentSection_pqr.x ==-100? 'opacity:1' :'opacity:0'" style="display: flex;transition: all ease .5s; align-items: end; gap: 1rem">
                <div class="date" >
                    <h6> <b> Desde</b> </h6>
                    <Calendar v-model="temp_start_date"     dateFormat="dd-mm-yy" 
                    ></Calendar>
                </div>

                <div class="date">
                    <h6><b>Hasta</b> </h6>
                    <Calendar :disabled="!temp_start_date" v-model="temp_end_data"     dateFormat="dd-mm-yy"  
                    ></Calendar>
                </div>
                
                
                <Button @click="obtain_pqr_report(temp_start_date,temp_end_data)" severity="help" label="Buscar" icon="pi pi-search"></Button>
            </div>

            <div>
                <h6 class=""> <b>MODOS</b>  </h6>
                <Dropdown style="width: 20rem;" optionLabel="name" placeholder="Avanzado" v-model="login.currentSection_pqr" :options="options"></Dropdown>
            </div>
