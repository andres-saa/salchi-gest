  ...
  <div class="home">
    ...
    <div v-if="filteredModel.length" class="sections">
      ...
      >
        ...
        <div
          @click="collasable[menu.label] = !collasable[menu.label]"
          v-if="visibleCountMenu(menu) > 8"
          style="position: absolute;bottom: 0;padding-top: 4rem; display: flex;justify-content: center;width: 100%;"
          :style=" `background:${menu.color}`"
        >
          <Button
            text icon="pi pi-angle-down"
...
// --- Modelo filtrado y ordenado ---
const filteredModel = computed(() => {
  const tree = filterTree(store.model)   // respeta permisos + búsqueda
  return sortTreeByVisibleItems(tree)    // ordena de mayor a menor
})

/* ============ AGREGAR A store.open ============ */
const ensureArray = (x) => (Array.isArray(x) ? x : [])
const addToOpen = (item, parent) => {
  if (!item || (!item.to && !item.url)) return
  if (!Array.isArray(store.open)) store.open = []

  const key = item.to ?? item.url
  const idx = store.open.findIndex((it) => (it.to ?? it.url) === key)
  ...
  const payload = {
    ...
    url: item.url ?? null,
    icon: item.icon ?? null,
    color: item.color ?? parent?.color ?? 'var(--primary-color)',
    permision_id: item.permision_id ?? null,
    ts: Date.now()
  }

  if (idx === -1) {
    store.open.push(payload)
  } else {
    // si ya existe, actualiza info (útil si cambia el color/icon)
    store.open.splice(idx, 1, { ...store.open[idx], ...payload })
  }

  // (Opcional) limitar historial
  // const N = 30; if (store.open.length > N) store.open.splice(0, store.open.length - N)
}

const onLinkClick = (node, parent) => {
  addToOpen(node, parent)
}
