<template>
  <div class="col-12 m-auto p-0">
    ...
    <div class="col-12" v-if="charging"
      style="display: flex;flex-direction: column; pointer-events: none; align-items: center; justify-content: center; position: fixed;z-index: 1000;left: 0;top: 0; height: 100%;background-color: rgba(0, 0, 0, 0.5);">


      <p class="text-3xl" style="font-weight: bold; color: white; text-shadow: 0 0 10px rgba(0, 0, 0, 0.551);">CARGANDO
        PERMISOS</p>
    ...
    <DataTable ref="dt" :value="permisos" v-model:selection="selectedPermisos" dataKey="id" :paginator="true" :rows="10"
      :filters="filters"
      ...
      <Column class="p-2" field="tipo" header="Tipo" :sortable="true" headerStyle="width:10rem; min-width:min-content; ">
        <template #body="permiso">
          <span class="p-column-title">Code</span>
          <!-- {{ permiso.data.id }} -->
          {{ permiso.data.tipo }}
      ...
      <Column class="p-2" field="location" header="Acciones" :sortable="true" frozen alignFrozen="right"
        ...
        <template #body="permiso">
          ...
          <div style="display: flex; gap: 1rem;">
            <Button class="p-0" outlined severity="info" label="Small" @click="open(permiso.data)" text> <i
                class="text-xl" style="font-weight: bold;" :class="PrimeIcons.EYE"> </i> </Button>
            <Button class="p-0" :disabled="permiso.data.status.status == 'aprobado'" outlined severity="success"
              label="Small" text @click="openAceptar(permiso.data)" style="display: flex;justify-content: center;"> <i
                class="text-xl" style="font-weight: bold;" :class="PrimeIcons.CHECK"></i> </Button>
            <Button class="p-0"
              :disabled="permiso.data.status.status == 'rechazado' && permiso.data.status.status != 'aprobado'" outlined
              severity="danger" label="Small" @click="openRechazar(permiso.data)"
              style="display: flex;justify-content: center;" text> <i class="text-xl" style="font-weight: bold;"
  ...
  <Dialog style="border: none; box-shadow: none;" v-model:visible="show" :style="{ width: '700px' }"
    header="Revizar permiso" :modal="true" class="p-fluid m-2 pt-8">
    ...
    <div class=" a4-size p-0 lg:p-6 mi-clase mt-4">
      ...
      <h6 class="text-center py-4" style="font-weight: bold;text-transform: uppercase;">FORMATO SOLICITUD DE PERMISO
        {{ currentPermiso.tipo }} <span style="text-transform: capitalize;"></span></h6>
      ...
      <h5 class="text-l "
        ...
        <p class="p-0 m-0">
          <span style="font-weight: bold;">Fecha de la solicitud: </span> <span
            style="border-bottom: 1px solid ; padding-bottom: 0; padding-right: 2rem"> {{
              convertirFechaALocal(currentPermiso.start_date) }}</span>

        </p>

        <p class="p-0 m-0" style="width: 100%;">
          <span style="font-weight: bold;">Nombre: </span> <span
            style="width: 100%; border-bottom: 1px solid ; padding-bottom: 0; padding-right:3rem">{{ currentUser.name
            }}</span>
    ...
    <div class=" col-12 md:col-6 m-auto p-0"
      style=" display: flex;gap: 1rem;; overflow: hidden; justify-content: space-around;position: absolute;top: 0rem;right: 0; background-color:rgba(255, 255, 255, 0)te;">
      <!-- <Button style="background-color: rgb(255, 255, 168);color: rgba(0, 0, 0, 0.864); border: none;" @click="open(permiso)"> REVISAR</Button> -->
      <Button class="text-center" @click="openAceptar(currentPermiso)"
        style="background-color: rgb(105, 255, 157);color: rgba(0, 0, 0, 0.864); border: none;display: flex; justify-content: center;">
        APROBAR </Button>
      <Button @click="openRechazar(currentPermiso)"
        style="background-color: rgb(255, 125, 125);color: rgba(0, 0, 0, 0.864); border: none;display: flex; justify-content: center;">
        RECHAZAR</Button>
    </div>
  </Dialog>
  ...
  <Dialog header="Confirmacion" v-model:visible="consfirmacionAceptar" :style="{ width: '350px' }" :modal="true">
    ...
    <Button @click="acceptPermission(currentPermiso)"
      style="margin:auto ; background-color:rgb(105, 255, 157) ; color: black;border: none;"
      class="m-auto my-4 text-center"> <span class="text-center col-12 p-0">Si, continuar</span></Button>
    <Button style="background-color: rgb(255, 156, 156);color: rgba(0, 0, 0, 0.864); border: none;"
      class="m-auto my-4 ml-4 text-center"> <span class="text-center col-12 p-0">No, talvez luego</span></Button>
  ...
  <Dialog header="Confirmacion" v-model:visible="consfirmacionRechazar" :style="{ width: '350px' }" :modal="true">
    ...
    <Textarea v-model="razonesDeRechazo" style="max-width: 100%; min-width: 100%;min-height: 100px;"
    ...
    <Button @click="rejectPermission(currentPermiso)"
      style="margin:auto ; background-color:rgb(105, 255, 157) ; color: black;border: none;"
      class="m-auto my-4 text-center"> <span class="text-center col-12 p-0">Si, continuar</span></Button>
    <Button style="background-color: rgb(255, 156, 156);color: rgba(0, 0, 0, 0.864); border: none;"
      class="m-auto my-4 ml-4 text-center"> <span class="text-center col-12 p-0">No, talvez luego</span></Button>
